...
import { Ticket, Printer, RotateCcw, Clock, Hash, CreditCard, BookOpen, FileText, User, Zap } from "lucide-react"
...
export function TokenCard({ token, onPrint, onNewToken }: TokenCardProps) {
  ...
  return (
    <div className={`rounded-xl border-2 bg-card p-6 shadow-sm ${isFastTrack ? "border-accent/30" : "border-primary/20"}`}>
      {/* Token Type Badge */}
      ...
      <div className="mb-5 flex flex-col items-center gap-3 border-b border-border pb-5">
        <div className={`flex h-14 w-14 items-center justify-center rounded-2xl ${isFastTrack ? "bg-accent/10" : "bg-primary/10"}`}>
          <Ticket className={`h-7 w-7 ${isFastTrack ? "text-accent" : "text-primary"}`} />
        </div>
        <div className="text-center">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Token Number</p>
          <p className={`mt-1 text-4xl font-bold tracking-tight font-mono ${isFastTrack ? "text-accent" : "text-primary"}`}>
            {token.tokenNumber}
          </p>
        </div>
      </div>
